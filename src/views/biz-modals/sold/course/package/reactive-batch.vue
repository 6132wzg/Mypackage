<template>
  <st-modal
    ...
    size="small"
    v-model="show"
    wrapClassName="modal-sold-card-reactive"
  >
    <st-form :form="form" labelWidth="75px">
      ...
      <st-form-item label="激活延期时间" required>
        <st-input-number
          :max="999"
          :min="1"
          @change="onChangeDate"
...
import { ReactiveBatchService } from './reactive-batch.service'
import { ruleOptions } from './reactive-batch.config'
import { BATCH_TYPE, BATCH_INFO } from '@/constants/common/batch-operation'
export default {
  name: 'ModalTaskReactiveBatch',
  serviceProviders() {
    return [ReactiveBatchService]
  },
  serviceInject() {
    return {
      reactiveBatchService: ReactiveBatchService
    }
  },
  rxState() {
    return {
      list_num: this.reactiveBatchService.list_num$,
      loading: this.reactiveBatchService.loading$
    }
  },
  props: {
    ids: {
  ...
  mounted() {
    this.reactiveBatchService.getPackageTaskNum(this.searchQuery).subscribe()
  },
  data() {
  ...
  methods: {
    ...
    onSubmit() {
      this.form.validate().then(values => {
        ...
        this.reactiveBatchService
          .taskReactiveInPackage({
