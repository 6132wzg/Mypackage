<template>
  <st-panel-layout :class="b()">
    <MarkteingPluginTitle pathName="brand-app-plugin-seckill-list" />
    <div :class="b('intro')" v-if="!info.is_open">
      ...
      <!-- <img
        :class="b('intro-item')"
        src="https://styd-frontend.oss-cn-shanghai.aliyuncs.com/images/saas-pro/plugin_seckill_banner_1%402x.png"
        alt=""
      />
      <img
        :class="b('intro-item')"
        src="https://styd-frontend.oss-cn-shanghai.aliyuncs.com/images/saas-pro/plugin_seckill_banner_2%402x.png"
        alt=""
      />
      <img
        :class="b('intro-item')"
        src="https://styd-frontend.oss-cn-shanghai.aliyuncs.com/images/saas-pro/plugin_seckill_banner_3%402x.png"
        alt=""
      />
      <img
        :class="b('intro-item')"
        src="https://styd-frontend.oss-cn-shanghai.aliyuncs.com/images/saas-pro/plugin_seckill_banner_4%402x.png"
        alt=""
      /> -->
    </div>
    <st-panel v-else app>
        ...
        >
          ...
          <span slot="operator" slot-scope="text, record">
            <st-table-actions sytle="width: 120px">
              <a
                @click="onEdit(record)"
                v-if="record.auth['brand:activity:limit_time_spike|edit']"
              >
                编辑
              </a>
              <a
                @click="onStop(record)"
                v-if="record.auth['brand:activity:limit_time_spike|stop']"
              >
                结束
              </a>
              <a
                @click="onDelete(record)"
                v-if="record.auth['brand:activity:limit_time_spike|delete']"
              >
                删除
              </a>
            </st-table-actions>
...
export default {
  ...
  bem: {
    b: 'page-brand-app-plugin-seckill',
    bHeader: 'page-header'
  },
  serviceInject() {
    return {
  ...
  methods: {
    ...
    onStop(record) {
      ...
      this.$confirm({
        ...
        content: '确定停止该活动吗？',
        onOk() {
          that.service.stopSeckill(record.activity_id).subscribe(res => {
            that.$router.reload()
          })
        },
        onCancel() {}
    ...
    onDelete(record) {
      ...
      this.$confirm({
        ...
        content: '确定删除该活动吗？',
        onOk() {
          that.service.deleteSeckill(record.activity_id).subscribe(res => {
            that.$router.reload()
          })
        },
        onCancel() {}
