  ...
  <st-modal :title="title" v-model="show" @ok="onSubmit" @cancel="onCancel">
    <div>
      ...
      <st-container>
        <st-table
          rowKey="activity_id"
          :loading="loading.getGroupBuyList"
          :columns="columns"
          :dataSource="list"
          :page="page"
          :rowSelection="{
...
<script>
import { GroupBuycolumns as columns } from './group-table-select.config'
import { GroupTableSelectService } from './group-table-select.service'
...
export default {
  ...
  serviceInject() {
    return {
      ...
      service: GroupTableSelectService
    }
  },
  rxState() {
    return {
  ...
  methods: {
    ...
    getList() {
      console.log('我是新的拼团')
      this.service.getGroupBuyList(this.searchQuery).subscribe()
    },
    onSubmit() {
      const data = {
