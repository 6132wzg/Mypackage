...
export class StaffService {
  ...
  auth$ = this.authService.authMap$({
    export: 'shop:stat:sales:summary|export_sales'
  })
  constructor(
  ...
  SET_LIST(list: any) {
    ...
    list = list.map((item, index) => {
      ...
      if (index < 1) expendKeys = [...expendKeys, item.staff_id.toString()]
  ...
  getStaffList(query: AllStaffListParams) {
    return this.commonApi.getShopStaffList(query).pipe(
      tap(res => {
        const staffList: any = []
        res.list.map(item => {
          staffList.push({
