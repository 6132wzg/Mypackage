...
<script>
import { ExtendDayBatchService } from './extend-day-batch.service'
...
export default {
  ...
  serviceProviders() {
    return [ExtendDayBatchService]
  },
  serviceInject() {
    return {
      extendDayBatchService: ExtendDayBatchService
    }
  },
  rxState() {
    return {
      list_num: this.extendDayBatchService.list_num$,
      loading: this.extendDayBatchService.loading$
    }
  },
  props: {
    ids: {
  ...
  mounted() {
    this.extendDayBatchService.getPackageTaskNum(this.searchQuery).subscribe()
  },
  data() {
