      ...
      <st-form :form="form" labelWidth="88px">
        <div :class="refund('refund')">
          ...
          <st-form-item label="退款金额" required>
            <st-input-number
              :max="Number(packageInfo.pay_price)"
              :float="true"
          ...
          <st-form-item v-if="packageInfo.gift_points">
            <div :class="refund('integral')">
              该订单赠送{{ packageInfo.gift_points }}积分，退款时需扣回
              <span style="width: 150px;padding-left: 10px">
                <st-input-number
                  width="150px"
                  type="number"
                  :max="+packageInfo.gift_points"
                  placeholder="请输入"
...
export default {
  ...
  created() {
    this.refundService.getPackageInfo(this.id, this.type).subscribe(res => {
  ...
  methods: {
    ...
    handleIntegralCheck(rule, value) {
      if (value > this.packageInfo.member_points) {
        return `该用户仅剩${this.packageInfo.member_points}积分，不足扣减`
      }
      return true
    }
  }
