  ...
  <st-modal :title="title" v-model="show" @ok="onSubmit" @cancel="onCancel">
    <div>
      ...
      <st-container>
        <st-table
          rowKey="activity_id"
          :loading="loading.getSeckillList"
          :columns="columns"
          :dataSource="list"
          :page="page"
          :rowSelection="{
...
<script>
import { Seckillcolumns as columns } from './seckill-table-select.config'
import { SeckillTableSelectService } from './seckill-table-select.service'
...
export default {
  ...
  serviceInject() {
    return {
      ...
      service: SeckillTableSelectService
    }
  },
  rxState() {
    return {
  ...
  methods: {
    ...
    getList() {
      console.log('我是新增的秒杀')
      this.service.getSeckillList(this.searchQuery).subscribe()
    },
    onSubmit() {
      const data = {
